<template>
  <section class="text-white mt-24" id="certificates">
    <div class="px-4 xl:pl-16">
      ...
      <ul
        class="px-4 sm:py-16 xl:pr-16 pb-0 grid grid-cols-1 gap-6 pt-10 sm:grid-cols-2 md:gap-10 md:pt-12 lg:grid-cols-3"
        data-aos="fade-right"
        data-aos-once="true"
      >
        <div v-for="project in filteredCertificates" :key="project.id">
          ...
          >
            <div
              class="overlay items-center justify-center absolute top-0 left-0 w-full h-full bg-[#181818]/50 bg-opacity-0 transition-all duration-500"
              :class="{
                'flex bg-opacity-80': showIcon === project.id,
...
const filteredCertificates = computed(() => {
  ...
  return Certificates.value.filter(
    (project) => project.certificate.toLowerCase() === selectedCert.value.toLowerCase()
  );
});
